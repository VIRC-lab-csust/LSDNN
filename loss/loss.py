...
class CrossEntropyMate(object):
    
    def __init__(self, weight=[0.2, 0.2, 0.3, 0.3]):
        weight = torch.from_numpy(np.array(weight)).float().cuda()
        self.loss_f = nn.CrossEntropyLoss(weight=weight, reduction='mean')

    def __call__(self, vector, label):
        return self.loss_f(vector, label)
